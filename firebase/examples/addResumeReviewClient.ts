// Example: Add a resume review document using Firebase v9 modular SDK (client-side)
...
// Exact collection name as required by the new issue
export const RESUME_REVIEW_COLLECTION = "resume review" as const;

// Example document
export const exampleResumeReview = {
  resumeId: "res_12345",
  candidateName: "Aisha Kumar",
  userId: "user_987",
  resumeFileUrl:
    "https://firebasestorage.googleapis.com/v0/b/<bucket>/o/resumes%2Fres_12345.pdf?alt=media",
  parsedSections: {
    ...
    education: [{ school: "VIT", degree: "B.Tech CSE", year: 2021 }],
  ...
  automatedFeedback: [
    {
      type: "keyword",
      message: "Strong alignment with React/TS stack",
      confidence: 0.92,
    },
  ...
  reviewerFeedback: [
    ...
    {
      reviewerId: "rev_001",
      comment: "Looks good for frontend roles",
      rating: 4,
      timestamp: serverTimestamp() as unknown as Timestamp,
    },
  ],
  suggestions: [
    "Add measurable impact to bullet points",
...
export async function addResumeReviewClient(doc: typeof exampleResumeReview) {
  ...
  const colRef = collection(db, RESUME_REVIEW_COLLECTION);
  const ref = await addDoc(colRef, payload);
  return ref.id;
}

// Usage (ensure user is authenticated and has role):
// const id = await addResumeReviewClient(exampleResumeReview);
// console.log('Inserted resume review id:', id);
